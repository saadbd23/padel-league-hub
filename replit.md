...
## November 26, 2025 (Current)
- **Fixed Players Leaderboard - All Round 1 Players Now Showing**: Completed comprehensive fix to players leaderboard. Issue was incomplete player record creation and missing stat calculation for all 44 Round 1 participants (40 from 10 completed matches + 4 from 1 walkover + 3 approved substitutes). Created Player records for all team members and approved substitutes. Updated leaderboard query to display all players with any match history (wins, losses, draws, or matches_played > 0). Recalculated stats for all completed Round 1 and Round 2 matches. Substitutes (Fuad Nasir, Asef Kamal Pasha, Nahian Shabab) now appear on leaderboard as they accumulate stats from approved match participation.
- **Updated Substitute Approval Flow**: When substitutes are approved, system automatically creates Player record for them so they can accumulate individual stats for future league plays and appear on players leaderboard.

## November 25, 2025
- **Added Admin Notification Emails for Requests**: Implemented admin email notifications when teams submit substitute or reschedule requests. Admin receives detailed notifications including team names, match details, proposed date/time, substitutes info, deadline information, and current usage statistics. Emails are sent to the ADMIN_EMAIL environment variable with distinctive subject lines (🔔 icons) to help admin quickly identify pending requests needing approval.
- **Fixed Match Management Round Completion Counter**: Updated Match Management section to count both "completed" and "walkover" statuses in round completion display. Now correctly shows "11/11 Complete" for Round 1 (matching Round Summary display). Previously showed "10/11" because walkover matches weren't being counted in the template. Also added orange badge styling for walkover matches to match Round Summary visual consistency.
- **Removed League-Ladder Cross-Check Restrictions**: Modified registration validation to allow League players to join Men's and Women's Ladder divisions as teams and free agents. Specifically removed checks preventing league team members from registering in ladder teams/free agents for Men's/Women's divisions (Mixed division still maintains restriction that allows flexibility). Validation now only checks within ladder system (ladder teams cannot have duplicate players, ladder free agents cannot duplicate). This enables league players to participate in ladder competitions simultaneously.
- **Simplified Round Summary Score Display**: Modified admin panel's Round Summary section to display only one set of scores instead of both team perspectives. Changed from "6-0, 6-0 vs 0-6, 0-6" format to "6-0, 6-0" for cleaner, more compact table view. Scores still show confirmation status (✓ confirmed, ⏳ awaiting confirmation).
